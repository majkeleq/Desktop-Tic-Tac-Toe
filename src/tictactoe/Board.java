...
public class Board extends JPanel {
    ...
    StatusLabel statusLabel;//reference
    Player player1;
    Player player2;
        ...
        for (int i = 3; i > 0; i--) {
            for (int j = 0; j < 3; j++) {
                ...
                board[i - 1][j].addActionListener(e -> {
                    synchronized (lock) {
                        if (e.getSource() instanceof Cell clickedCell && TicTacToe.getGameState().equals(TicTacToe.State.IN_PROGRESS)) {
                            if (isNextMoveX) {
    ...
    public void makeMoveIfRobot() {
        ...
        if (TicTacToe.getGameState().equals(TicTacToe.State.IN_PROGRESS)) {
    ...
    public void startBoard(String pl1, String pl2) {
        ...
        TicTacToe.setGameState(TicTacToe.State.IN_PROGRESS);
        statusLabel.setStatusLabel();
        if (pl1.equals("Human")) {
